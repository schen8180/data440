...
def main():
    ...
    wordlist = []  # * wordlist is a list of the non-stopwords
    ...
    # popularlist = []
    # sorted_items = sorted(sumcounts.items(), key=lambda x: x[1], reverse=True)
    # first_500_largest = sorted_items[:500]
    # for key, value in first_500_largest:
    #     popularlist.append(key)
    # popularlist
    ...
    for key, value in sumcounts.items():
        words = str(value).split()
        filtered_words = [word for word in words if word.lower() not in stop_words]
        sumcounts[key] = ''.join(filtered_words)
    sorted_items = sorted(sumcounts.items(), key=lambda x: x[1], reverse=True)
    first_500_largest = sorted_items[:500]
